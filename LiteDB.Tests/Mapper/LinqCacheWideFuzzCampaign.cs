...
namespace LiteDB.Tests.Mapper
{
    internal sealed class LinqCacheWideFuzzCampaign
    {
        ...
        private readonly List<Failure> _samples = new List<Failure>();
        ...
        private void Record(string kind, int seed, int valueSeed, LambdaExpression query, string detail)
        {
            ...
            lock (_samplesLock)
            {
                _samples.Add(failure);
                if (_samples.Count > SampleLimit)
                {
                    var worst = _samples.OrderBy(x => x.Seed).ThenBy(x => x.ValueSeed)
                        .ThenBy(x => x.Kind, StringComparer.Ordinal).ThenBy(x => x.Detail, StringComparer.Ordinal).Last();
                    _samples.Remove(worst);
                }
            }
        }
        ...
        private void Write(CampaignSummary summary)
        {
            ...
            foreach (var failure in _samples.OrderBy(x => x.Seed).ThenBy(x => x.ValueSeed)
                .ThenBy(x => x.Kind, StringComparer.Ordinal).ThenBy(x => x.Detail, StringComparer.Ordinal))
