...
namespace LiteDB.Tests.Issues
{
    public class Issue2211_Tests
    {
        ...
        [InlineData(false)]
        [InlineData(true)]
        public void LiteDatabase_Should_Persist_In_Path_With_Equal_Sign(bool viaConnectionString)
        {
            var filename = GetTempDatabasePathWithEqualSign();

            try
            {
                using (var db = viaConnectionString
                    ? new LiteDatabase(new ConnectionString(filename))
                ...
                {
                    Assert.Equal("correct path", db.GetCollection("rows").FindById(7)["value"].AsString);
                }
            }
            finally
            {
