...
namespace LiteDB.Tests.Issues
{
    public class Issue1644_Tests
    {
        ...
        [InlineData(false)]
        [InlineData(true)]
        public void Deferred_compilation_keeps_construction_mode_across_interpreter_scopes(bool interpret)
        {
            var previous = RuntimeExpression.ForceInterpretation;
            var previousObserver = RuntimeExpression.CompilationObserver;
            try
            {
                RuntimeExpression.ForceInterpretation = interpret;
                var selectedMode = RuntimeExpression.UseInterpretation;
                var expression = BsonExpression.Create("mode_" + Guid.NewGuid().ToString("N") + " = 1 OR true");
                RuntimeExpression.ForceInterpretation = !interpret;
                var observed = new List<bool>();
                RuntimeExpression.CompilationObserver = mode => observed.Add(mode);
                expression.ExecuteScalar().AsBoolean.Should().BeTrue();
                observed.Should().Equal(selectedMode);
            }
            finally
            {
                RuntimeExpression.CompilationObserver = previousObserver;
                RuntimeExpression.ForceInterpretation = previous;
            }
        }
    }
}
