...
namespace LiteDB.Internals
{
    public class WalTransactionBoundary_Tests
    {
        ...
        public void InterleavedTransactions_KeepTheirOwnSlots_AndRecoverInCommitOrder(string password, bool rollbackFirst)
        {
            ...
            first.TransactionID.Should().BeGreaterThan(secondID,
                "a delayed WAL writer must advance past a transaction that wrote first");
        ...
        [InlineData(null)]
        [InlineData("secret")]
        public void DelayedCommit_RebasesEarlierSafepointsBeforeConfirmation(string password)
        {
            using var test = new WalTestDatabase(password);
            test.Seed("first");
            test.Seed("second");
