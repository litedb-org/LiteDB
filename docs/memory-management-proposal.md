...
2. **Parameterized `Query.*` helpers are an opt-in API, not a change to the
   ...
   - The opt-in entry point (`Query.Parameterized`, comparison
     ...
     a parameterized expression such as `{field} = @__queryValue0` (the helper
     picks a collision-free parameter name, so the exact name is not a
     contract); the source text is constant per field and operator. The LINQ visitor already emits
