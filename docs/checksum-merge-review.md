...
| --- | --- | --- |
| Does adopting v10 still require database-sized I/O or WAL space? | `FiveHundredGiBCutoverUsesBoundedHeaderIoAndWalSpace` rejects reads/writes outside page zero on a 500 GiB sparse model, plain/encrypted. | One 8 KiB data write and a peak 32 KiB WAL, excluding encryption preamble; draining an existing legacy WAL remains normal checkpoint work. |
| Can old and new pages coexist safely? | `CutoverChangesOnlyHeader_ThenWritesMigratePagesOutOfOrder` covers v8/v9, untouched ciphertext, cold legacy pages, hot converted pages, newly allocated pages, index queries, read-only reopen, and rebuild's reader. | Marker and authenticated legacy boundary govern each page; all WAL frames use the new format. |
| Can a marker bit flip disable validation or enable an unknown format? | `MixedCoverageRejectsEveryUnknownMarkerAndEverySingleBitDowngrade`, `HeaderAndPagesPastLegacyBoundaryAlwaysRequireChecksums`, `ExtendedDataPageIsRejectedBeforeParsingOrModification`, and `WalRequiresCurrentPageMarkerEvenWithAValidFrameCrc`. | Every single-bit marker change fails closed, unknown/extended formats fail even with valid CRCs, and old-range checked pages still require their CRC. |
| Are mixed permissions trusted without header integrity? | `CoverageMetadataFailsClosedEvenWithAValidHeaderCrc`, plus per-byte header corruption tests. | Only validated coverage values and an in-range legacy bound are accepted from a validated header. |
| Do lazy conversion, rollback, page reuse, and torn writes compose? | `TornMixedCheckpointRecoversConvertedAndUntouchedPages` and `RollbackDoesNotConvertPages_AndReusedLegacyIdsStayChecksummed`. | Plain/encrypted mixed databases recover all documents and secondary-index results after tested write cuts, preserve read-only images, and checkpoint/reopen. Rollback leaves legacy pages intact; reused IDs are checksummed. |
| How does coverage become Complete? | `ExplicitRebuildCompletesMixedCoverage`. | Explicit rebuild creates and reopens a fully checksummed file. Normal writes migrate out of order; coverage remains conservatively Mixed without a scan. |
...
| Can lazy cutover resume after torn header writes? | `LazyConversion_CanResumeAtEveryHeaderWriteBoundary` covers v8/v9 and cuts inside encrypted blocks. | One header write publishes cutover; verified header redo restores it before conversion is retried. |
...
The global WAL cutover is atomic, while data pages migrate lazily. Mixed coverage
does not protect untouched legacy payloads; diagnostics expose that limitation.
No background migrator or automatic full-coverage scan is introduced.
