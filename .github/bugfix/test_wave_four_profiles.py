"""Compact profiles and delivered role requirements for four prepared issues."""
...
COUNTS = {2746: (1, True), 2807: (1, False), 1829: (2, False), 1444: (2, True)}
NOTES = {2746: {"behavior": "INVALID_COLLECTION_NAME", "compatibility": "checkpoint and reopen"},
         2807: {"behavior": "Query.And/Or/Not", "lifecycle": "shared mutable"},
         1829: {"behavior": "parameter identity"},
         1444: {"behavior": "#2874", "compatibility": "old ObjectId indexes"}}


class WaveFourProfiles(unittest.TestCase):
    ...
    def test_object_id_compatibility_obligation_cannot_be_reduced_to_fresh_files(self):
        text = " ".join(self.definition(1444)["review_requirements"]["compatibility"])
        for phrase in ("signed timestamp ordering", "pre/post-2038", "lookup", "range traversal",
                       "rebuild or migration", "fresh database", "generic vector compatibility",
                       "public Timestamp API type/ABI"):
            self.assertIn(phrase, text)

    def test_issue_path_pair_does_not_grant_another_issue_a_compact_profile(self):
        for number in (2807, 1829, 1444):
            definition = self.definition(number)
