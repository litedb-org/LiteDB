...
class AutoIdCorepairProfileTests(unittest.TestCase):
    ...
    def test_prior_atomicity_obligation_survives_superseding_campaign(self):
        ...
        for phrase in ("a98b29089c01e21e50856abd0cc9bbea1ecc5631", "public int? Id { get; }",
                       "implicit and explicit transactions", "caller Commit", "mixed/lazy batches",
                       "non-null custom setter that throws", "Setter != null guard alone",
                       "three new reviews", "write-then-delete"):
            self.assertIn(phrase, lifecycle)
        behavior = " ".join(contract["review_requirements"]["behavior"])
        for phrase in ("LiteException", "read-only/setter", "class-drift allowance"):
