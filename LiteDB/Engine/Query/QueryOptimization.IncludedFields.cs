...
namespace LiteDB.Engine
{
    internal partial class QueryOptimization
    {
        ...
        {
            return IndexExpressionIdentity.Matches(indexSource, expression);
        }
        ...
        private static bool TryGetMemberPath(Expression expression, string scope, List<string> path)
        {
            ...
            if (call.Method.Name == "ARRAY_FILTER" && call.Arguments[1] is ConstantExpression position &&
                position.Value is int arrayIndex && arrayIndex == int.MaxValue)
                return TryGetMemberPath(call.Arguments[0], scope, path);
            if (call.Method.Name == "MAP" && call.Arguments[4] is ConstantExpression mapping &&
                mapping.Value is BsonExpression body)
                return TryGetMemberPath(call.Arguments[3], scope, path) && TryGetMemberPath(body.Expression, "current", path);
            return false;
        }
