    ...
    public partial class BsonMapper
    {
        ...
        private static bool MatchesDeclaredMember(Type implementation, MemberMapper candidate, MemberMapper declared)
        {
            ...
            if (declared.ReflectedMember is PropertyInfo baseProperty && candidate.ReflectedMember is PropertyInfo implementationProperty)
            {
                ...
                return IsSameMember(GetVirtualDefinition(accessor), GetVirtualDefinition(target));
            }
            return IsSameMember(candidate.ReflectedMember, declared.ReflectedMember);
        }

        // An inherited non-virtual member reflected through the derived type is a
        // different MemberInfo instance than the one reflected through its base.
        private static bool IsSameMember(MemberInfo left, MemberInfo right)
        {
            if (left == null || right == null) return left == right;
            return left.DeclaringType == right.DeclaringType && left.Module == right.Module &&
